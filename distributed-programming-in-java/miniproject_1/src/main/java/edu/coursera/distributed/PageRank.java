...
public final class PageRank {
  ...
  public static JavaPairRDD<Integer, Double> sparkPageRank(
    ...
    return sites
        .join(ranks)
        .flatMapToPair((Tuple2<Integer, Tuple2<Website, Double>> t) -> {
          Website website = t._2._1;
          Double rank = t._2._2;

          List<Tuple2<Integer, Double>> results = new ArrayList<>(website.getNEdges());
          Iterator<Integer> iter = website.edgeIterator();
          while (iter.hasNext()) {
            Integer curr = iter.next();
            results.add(new Tuple2<>(curr, rank / website.getNEdges()));
          }
          return results;
        })
        .reduceByKey((a, b) -> a + b)
